...
def train():
    ...
    详细步骤：
        - 检测数据集目录结构，生成 data.yaml
        - 根据显存选择模型 (yolo11n/s/m) 和 imgsz/batch
        - 判断是否存在断点 last.pt，若存在则继续训练（学习率降低）
        - 否则如果存在已有 best.pt，则加载权重进行微调
        - 否则使用预训练模型从头训练
        - 训练参数经过精细调优（增强、优化器、余弦退火等）
        - 训练结束后复制 best.pt 到 models/ 目录，并运行验证输出 JSON 指标
    """
    print("=" * 62)
    ...
    # 阈值与参数经过实践调优，平衡速度与精度
    if total_gb >= 10:
        cfg = {"model_name": "yolo11m.pt", "imgsz": 800, "batch": 12}  # ≥10G: 中模型 + 高分辨率
    elif total_gb >= 7:
        cfg = {"model_name": "yolo11m.pt", "imgsz": 640, "batch": 8}   # ≥7G: 中模型 + 标准分辨率
    elif total_gb >= 5:
        cfg = {"model_name": "yolo11s.pt", "imgsz": 640, "batch": 16}  # ≥5G: 小模型 + 大批量
    else:
        cfg = {"model_name": "yolo11n.pt", "imgsz": 640, "batch": 8}   # <5G: 超轻模型

    print(f"   🎯 自动选择: {cfg['model_name']}  imgsz={cfg['imgsz']}  batch={cfg['batch']}")

    # 定义类别映射并生成 data.yaml（YOLO 训练所需的数据集描述文件）
    ...
    if os.path.exists(last_pt):
        try:
            # 权重文件可能包含训练元信息，通过 epoch 判断已完成轮数
    ...
    results = model.train(
        ...
        imgsz=cfg["imgsz"],
        batch=cfg["batch"],
        device=0,                  # 指定 GPU 设备 0
        workers=4,                 # 数据加载子进程数
        patience=40,               # 早停耐心值：验证指标 40 轮不提升则停止
        lr0=lr0,                   # 初始学习率
        lrf=lr0 * 0.1,             # 最终学习率 = 初始学习率 × 0.1（余弦退火终点）
        optimizer="AdamW",         # AdamW 优化器（较 SGD 更稳定）
        warmup_epochs=5,           # 学习率预热轮数，逐步提升 lr 防止早期震荡
        cos_lr=True,               # 余弦退火学习率调度
        close_mosaic=20,           # 最后 20 轮关闭 Mosaic 增强以提高精度
        augment=True,              # 启用数据增强
        hsv_h=0.02,                # HSV 色相增强幅度
        hsv_s=0.8,                 # HSV 饱和度增强
        hsv_v=0.5,                 # HSV 明度增强
        degrees=15.0,              # 随机旋转角度范围 (±15°)
        translate=0.15,            # 随机平移比例
        scale=0.6,                 # 随机缩放比例
        fliplr=0.5,                # 水平翻转概率
        mosaic=1.0,                # Mosaic 增强启用概率
    ...
    # 提取核心评估指标
    map50   = float(met.box.map50)    # IoU@0.5 平均精度
    map_all = float(met.box.map)      # IoU@0.5:0.95 平均精度
    prec    = float(met.box.mp)       # 精确率
    rec     = float(met.box.mr)       # 召回率
